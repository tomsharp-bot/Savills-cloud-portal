...
Open the form in a phone browser at `https://savillscloudportal.co.uk/HHSRS-site-form` (or `http://localhost:3000/HHSRS-site-form` locally). No login for surveyors in v1. Photos are stored in the private Spaces bucket under `hhsrs-site-form/{submissionId}/{file}` (a local file is only a cache). HEIC/HEIF is converted to JPEG on upload. If that upload fails, the issue is not saved and the surveyor is asked to try again. When the office marks the case as actioned, a copy is placed in Spaces under `HHSRS - Completed/<project>/<UPRN> - <address>/<file>`, reading the cache or the site-form object. If that copy is unavailable, the case still moves to the Main Log and the copy can be retried. **1 to 4 photos**, **each photo up to 40MB** (JPEG, PNG, WebP, HEIC/HEIF). The browser compresses JPEG/PNG/WebP before upload when it can. Once project, survey date and surveyor name are filled, every remaining box is required, including any other details. House number stays lookup-only. Projects that need a client call reference require that reference, or a tick for Couldn't get through plus a note of why it is blank. That path stores a blank call reference, call outcome **Attempted**, and the note as call notes. Reporter Review and Generate email use those fields on the existing Call outcome bullet.
...
- **Photo Storage** (admin) at `/photos` — current project tiles (Photos Pool, Photo Folders, Create Photos Extract, Client Access → Completions), then an **HHSRS - Completed** section. The Photos Pool grid starts with photos added in the last 7 days (**Show all** lists the rest; search covers the whole pool). The enlarged photo has an admin **Blur** tool that pixelates rectangles and replaces the stored image in place. A morning job can upload into the pool with `PHOTO_INGEST_KEY` (`docs/photo-ingest.md`). Open a completed project for a numerical UPRN list, then that property’s photos. **Find by UPRN or address** at the top searches the current store and HHSRS - Completed; matches sit at the bottom of the page. Pool thumbs stay coloured placeholders until DigitalOcean Spaces (`cloud-portal-vault`, LON1) credentials are set. HHSRS copies use that same Spaces client and are shown through the signed-in admin session. Only cases logged from the deploy onward are copied. `scripts/backfill-hhsrs-completed-photos.ts` can retry failures (`--retry-failed`) or copy older Main Log cases (`--backfill`); with no flag it copies nothing.
- **Projects Programme** at `/projects-programme` — surveyor × week board. Every **admin** can open it. **Surveyors and clients cannot** (no nav item; the route is forbidden). Only Tom Sharp’s admin login can change it: username `tsharp` or email `tsharp@savillshousing.co.uk` (see `PROGRAMME_EDITOR_IDS`). Other admins get a view-only board; save, tile edits, and survey-type edits are rejected on the server. With `BASE_PATH=/projectprogress` the live URL is `https://savillscloudportal.co.uk/projectprogress/projects-programme` (hub: `https://savillscloudportal.co.uk/projectprogress/admin`). Bottom tables read Project Progress: current, upcoming, and the 5 most recently archived. Current and Upcoming show Nr of Weeks (distinct main-grid week columns, short stamp or full name) and Approx surveys (weeks × 40). Survey-type text starts from each project's survey-type ticks and is stored on edit. The grid itself is one shared board in Postgres.
...
- Spaces bucket **`cloud-portal-vault`** (LON1, `https://lon1.digitaloceanspaces.com`) is required for Reference Documents, HHSRS site-form photos, and HHSRS - Completed photo copies. The same `SPACES_*` variables cover all three. Photos Pool still shows demo placeholders until those variables are set. A failed HHSRS completed-copy does not stop Logging. A failed site-form photo upload does not save the issue. Reference documents and site-form photos are not left only on local disk.
