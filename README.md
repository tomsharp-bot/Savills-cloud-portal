...
- Spaces bucket **`cloud-portal-vault`** (LON1, `https://lon1.digitaloceanspaces.com`) is required for Reference Documents and for HHSRS - Completed photo copies. Photos Pool still shows demo placeholders until the same `SPACES_*` vars are set. A failed HHSRS copy does not stop Logging. Reference documents are not written to local disk.
